...
const RecipeDetail = () => {
  ...
  /* 재료 반복문 */
  const ingredient = ingList.map((ing, index) => (
    <div className="RecipeDetail-ingredientLine" key={index}>
      <p>{ing.name}</p>
      <p>
        {ing.amount}
        {ing.unit}
  ...
  const [thumbs, setThumbs] = useState({like});
  const handleLike = (e) => setThumbs(!thumbs);

  return (
    <div className="RecipeDetail" >
      <div className="RecipeDetail-title"  style={{ backgroundImage: `url(${image})`}}>
        <p>{name}</p>
        <p>좋아요</p>
